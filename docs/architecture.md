...
Data has three conceptual layers. **Authoritative data** contains human-controlled facts and operational records and cannot be directly modified by AI. **Derived intelligence** contains validated AI-assisted interpretations and must retain provenance to supporting authoritative records. **Presentation** contains regenerable copy or views and never becomes the factual source of truth. Gates 1–6 persist authoritative owner, mail, Project, scheduling, source-backed JobOpportunity, and Master Career Profile records. Gate 7 persists derived job-match intelligence that must resolve every positive match to Gate 6 career records.
...
Gate 7 compares a saved job description with the authoritative Master Career Profile. Deterministic logic owns requirement classification, match state, scoring, evidence resolution, stale detection, re-analysis, persistence, and authorization. A bounded interpreter may extract structured requirements and rank candidate evidence; it cannot invent career facts, assign the final percentage, or mutate Gate 6 records. Every positive or partial match stores relationships to real career record IDs. Resume generation, cover letters, and outreach remain prohibited.
...
Gate 5 implements the first step of the locked Job Search → Career Match → Decision Maker → Email Intelligence → Individualized Outreach roadmap in [future-career-outreach-workflow.md](future-career-outreach-workflow.md). Gate 7 implements Career Match, Gate 8 implements truthful tailored resumes, and Gate 9 implements the contact-intelligence foundation. Apollo organization enrichment, People API Search, and People Enrichment implement the existing organization/people/business-email boundaries when the server-only key is configured; the truthful provider-not-configured path remains active otherwise. A separate verification provider remains unconfigured. Outreach remains unimplemented. Shared services consume validated Project context; they do not create Project-specific copies of Mail, Compose, Job Search, Contacts, Email Discovery, Resume Studio, or AI systems.
...
Gate 9 adds independent `PeopleDiscoveryProvider`, `EmailDiscoveryProvider`, and `EmailVerificationProvider` boundaries. Deterministic application logic resolves the saved-job organization, derives a bounded role strategy from the target job, validates provider records, rejects wrong-company people, normalizes email states, ranks contacts with an explainable 100-point model, deduplicates only on provider-backed or exact person/company identities, and persists field-level provenance. The Apollo adapter enriches the organization before performing a title/seniority-bounded people search, enriches only the returned provider identities to obtain unobfuscated professional records and business-email status, rejects placeholders and cross-domain email, and never requests personal email or phone data. AI does not own identity, employment, email, verification, ranking, state, or preferred-contact selection.
