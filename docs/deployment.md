...
Gate 6 owner editing requires migrations `202608240010_master_career_profile.sql`, `202608250015_career_owner_editing.sql`, and `202608250016_fix_profile_history_target.sql`. Apply them before deploying the Career Profile editor. No new environment variable or external AI provider is required. Verify an authenticated edit produces a private `career_fact_history` entry, the edit persists after refresh, and anonymous reads/writes return no owner data. Never upload the source resume PDFs to public assets or commit temporary intake files.

Gate 6A additionally requires `202608250017_gate6a_candidate_fact_foundation.sql`. It reuses the existing reviewed resume evidence and requires no new API key or AI call. After migration, verify two successful `career_source_extractions`, persisted candidates, at least one exact low-risk auto-confirm where supported, a Review Needed exception, owner resolution/edit persistence, numbered versions, provenance, and anonymous denial. A deployment is incomplete if the database migration has not landed before the new `/app/career` page, because the page reads the new tables server-side.
