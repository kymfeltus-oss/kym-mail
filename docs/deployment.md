...
Calendar and consultations require migration `202608260026_calendar_consultations.sql` before deploying the matching routes. Generate a high-entropy `CALCOM_WEBHOOK_SECRET`, store it only in local/Vercel server environments, and configure the same value on the Cal.com webhook for `BOOKING_CREATED`, `BOOKING_RESCHEDULED`, and `BOOKING_CANCELLED` at `https://www.kymmailapp.com/api/webhooks/calcom`. In Cal.com, connect the real Google Calendar, create a free event and a hidden/private paid event, then save their HTTPS URLs through `/app/calendar`. Use a usage/date-limited Cal.com private link where the account plan supports it. Do not add Google Calendar scopes to KYM Mail and do not configure Cash App credentials.
