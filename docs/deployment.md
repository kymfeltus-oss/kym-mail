...
Gate 7 adds no external AI secret or resume-storage bucket. The configured deterministic provider reads owner-scoped Career Profile and Career Match records through the authenticated server boundary. DOCX and selectable-text PDF files are created on demand by a Node.js route, returned with private/no-store headers, and not persisted as public files. Migration `202608250013_tailored_resume_engine.sql` supplies the original foundation; apply the Gate 7 status and studio migrations before deploying the completed routes. If a future AI provider is added, its key must remain server-only and its strict structured output must pass the same factual validator before persistence.

Gate 8 requires migrations `202608250014_contact_intelligence.sql` and `202608260023_gate8_hiring_intelligence.sql` through `202608260025_gate8_private_equity_context.sql`. To enable live people research, set `APOLLO_API_KEY` only in `.env.local` and encrypted Vercel Development, Preview, and Production environments; never use a `NEXT_PUBLIC_` name. The key needs organization enrichment, People API Search, and People Enrichment. Confirm plan/scope access and credit limits before the single live acceptance search. Missing, invalid, rate-limited, quota-exhausted, timed-out, or malformed provider responses are reported safely and preserve stored research. Gate 8 makes no email-intelligence calls.

Calendar and consultations require migrations `202608260026_calendar_consultations.sql` through `202608260029_zelle_consultation_payments.sql` before deploying the matching routes. Generate a high-entropy `CALCOM_WEBHOOK_SECRET`, store it only in local/Vercel server environments, and configure the same value on the Cal.com webhook for `BOOKING_CREATED`, `BOOKING_RESCHEDULED`, and `BOOKING_CANCELLED` at `https://www.kymmailapp.com/api/webhooks/calcom`. In Cal.com, connect the real Google Calendar, create hidden/private 60-minute first-time and returning consultation events, then save their HTTPS URLs through `/app/calendar`. Use usage/date-limited Cal.com private links where the account plan supports them. Do not add Google Calendar scopes to KYM Mail and do not configure Zelle or bank credentials; only the public recipient name/contact is stored.

Production acceptance uses a clearly labelled no-funds test proof: submit at `/consult`, approve it in `/app/calendar`, open the tokenized link, create a real Cal.com booking, verify the signed webhook persists `BOOKED`, verify the event in Google Calendar, refresh both views, then exercise a second isolated request through rejection. Remove or cancel test appointments after recording results; do not fabricate a provider booking.
...
# Gate 7 deployment order

Deploy Gate 7 in dependency order: first apply `202608250021_gate7_resume_statuses.sql`, then `202608250022_gate7_interactive_resume_studio.sql`, then deploy the matching application build. The status migration is intentionally separate so PostgreSQL commits the new enum values before the schema migration uses them. Verify migration history with `supabase migration list --linked` and dry-run with `supabase db push --linked --dry-run` before the production push.

After deployment, authenticate as the owner, create/approve the Master Resume, open a real COMPLETE Career Match, and complete the documented Gate 7 browser journey. Anonymous share checks must use a logged-out context. An approved version may later become STALE but remains immutable; do not repair it in place. Re-run Career Match if needed and generate a new review version. No additional environment secret or provider credential is required for resume generation, PDF export, or sharing.
