...
import { CalendarClock, FolderKanban, Inbox, LayoutDashboard, LogOut, Send, SquarePen } from "lucide-react";
import { signOut } from "@/app/sign-in/actions";

type ActiveView = "dashboard" | "inbox" | "sent" | "scheduled" | "projects" | "compose";
const navigation = [
...
export function AppShell({ email, canSignOut, active = "dashboard", children }: { email: string; canSignOut: boolean; active?: ActiveView; children: React.ReactNode }) {
  return <div className="min-h-screen lg:grid lg:grid-cols-[276px_1fr]">
    ...
    <nav aria-label="Mobile primary" className="fixed inset-x-3 bottom-3 z-40 grid grid-cols-6 rounded-2xl border border-white/15 bg-[#183A5A]/95 p-2 text-white shadow-[0_18px_46px_rgba(24,58,90,.28)] backdrop-blur-xl lg:hidden">
