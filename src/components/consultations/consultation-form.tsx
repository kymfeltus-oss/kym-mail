...
export function ConsultationForm() {
  ...
  const [kind, setKind] = useState<ConsultationKind>("FIRST_TIME");
  ...
  if (statusUrl) return <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 sm:p-8"><CheckCircle2 className="size-9 text-emerald-700" /><h3 className="mt-5 text-xl font-semibold text-[#183A5A]">Payment proof received</h3><p className="mt-2 text-sm leading-6 text-[#526173]">Your proof is pending manual owner review. No booking access has been released yet.</p><a href={statusUrl} className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#183A5A] px-5 py-3 text-sm font-semibold text-white">View request status <ArrowRight className="size-4" /></a></div>;
  ...
  return <form onSubmit={submit} className="min-w-0 rounded-3xl border border-[#E8E2E3] bg-white p-5 shadow-[0_20px_60px_rgba(24,58,90,.08)] sm:p-8">
    ...
    <fieldset><legend className="text-sm font-semibold text-[#183A5A]">Consultation type</legend><div className="mt-2 grid gap-3 sm:grid-cols-2">{consultationKinds.map((optionKind) => { const offering = consultationOffering(optionKind); return <label key={optionKind} className={`cursor-pointer rounded-2xl border p-4 transition ${kind === optionKind ? "border-[#D95B72] bg-[#FFF3F4] ring-2 ring-[#F7DDE1]" : "border-[#D8D5D6]"}`}><input className="sr-only" type="radio" name="consultationKind" value={optionKind} checked={kind === optionKind} onChange={() => setKind(optionKind)} /><span className="block text-sm font-semibold text-[#183A5A]">{offering.name}</span><span className="mt-1 block text-xs text-[#64748B]">{offering.durationMinutes} minutes · {formatConsultationAmount(offering.priceCents)}</span></label>; })}</div><p className="mt-2 text-xs leading-5 text-[#7A8795]">The returning-client rate requires a completed first-time consultation and is checked against consultation history.</p></fieldset>
    <div className="mt-6 rounded-2xl bg-[#F8F5F4] px-4 py-3 text-sm text-[#526173]">Selected: <strong className="text-[#183A5A]">{selected.name} · {formatConsultationAmount(selected.priceCents)}</strong></div>
    <div className="mt-6 grid gap-5 sm:grid-cols-2">
      ...
      <label className="group sm:col-span-2"><span className="text-sm font-semibold text-[#183A5A]">Payment proof</span><span className="mt-2 flex min-h-32 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-[#D9A7B1] bg-[#FFF7F8] p-5 text-center transition group-focus-within:ring-4 group-focus-within:ring-[#F7DDE1]"><FileUp className="size-7 text-[#D95B72]" /><span className="mt-2 text-sm font-semibold text-[#183A5A]">Upload PNG, JPG, JPEG, or PDF</span><span className="mt-1 text-xs text-[#64748B]">Maximum 8 MB · stored privately</span><input name="paymentProof" required type="file" accept=".png,.jpg,.jpeg,.pdf,image/png,image/jpeg,application/pdf" className="mt-3 block w-full min-w-0 max-w-full text-xs text-[#64748B] file:mr-3 file:rounded-full file:border-0 file:bg-[#183A5A] file:px-4 file:py-2 file:font-semibold file:text-white" /></span></label>
    </div>
