...
export function ConsultationCalendarWorkspace({ settings, requests, upcoming }: { settings: ConsultationSettings | null; requests: ConsultationRequest[]; upcoming: ConsultationRequest[] }) {
  ...
  async function saveSettings(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/consultations/settings", {
      ...
      body: JSON.stringify({
        zelleRecipientName: form.get("zelleRecipientName"),
        zelleContact: form.get("zelleContact"),
        ...
        clientSessionBookingUrl: form.get("clientSessionBookingUrl"),
        clientSessionsActive: form.get("clientSessionsActive") === "on",
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <section className="mt-8 rounded-3xl border border-[#E8E2E3] bg-white p-5 sm:p-7"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-[#FFF3F4] text-[#D95B72]"><Settings2 className="size-5" /></span><div><p className="text-xs font-semibold uppercase tracking-[.18em] text-[#D95B72]">Booking Links</p><h2 className="text-xl font-semibold text-[#183A5A]">Consultation settings</h2></div></div><p className="mt-4 max-w-3xl text-sm leading-6 text-[#64748B]">Configure the two paid Cal.com events and the 15-minute paying-client session. Paid prices and durations are enforced by KYM Mail and cannot be edited here.</p>
      ...
      <form onSubmit={saveSettings} className="mt-6 grid gap-5 md:grid-cols-2"><label className="text-sm font-semibold text-[#183A5A]">Zelle recipient name<input name="zelleRecipientName" required maxLength={120} defaultValue={settings?.zelle_recipient_name ?? ""} className="mt-2 w-full rounded-xl border border-[#D8D5D6] px-4 py-3 font-normal" /></label><label className="text-sm font-semibold text-[#183A5A]">Zelle phone or email<input name="zelleContact" required maxLength={254} defaultValue={settings?.zelle_contact ?? ""} className="mt-2 w-full rounded-xl border border-[#D8D5D6] px-4 py-3 font-normal" /></label><label className="flex items-center gap-3 text-sm font-semibold text-[#183A5A] md:col-span-2"><input name="isActive" type="checkbox" defaultChecked={settings?.is_active ?? false} className="size-4 accent-[#D95B72]" /> Accept paid consultation submissions</label><label className="text-sm font-semibold text-[#183A5A] md:col-span-2">Payment instructions<textarea name="paymentInstructions" required minLength={10} maxLength={1000} rows={3} defaultValue={settings?.payment_instructions ?? ""} className="mt-2 w-full rounded-xl border border-[#D8D5D6] px-4 py-3 font-normal" /></label><label className="text-sm font-semibold text-[#183A5A] md:col-span-2">Reference instructions <span className="font-normal text-[#7A8795]">(optional)</span><input name="referenceInstructions" maxLength={500} defaultValue={settings?.reference_instructions ?? ""} className="mt-2 w-full rounded-xl border border-[#D8D5D6] px-4 py-3 font-normal" /></label><label className="text-sm font-semibold text-[#183A5A]">First-time consultation Cal.com URL<input name="firstTimeBookingUrl" required type="url" placeholder="https://cal.com/..." defaultValue={settings?.paid_booking_url ?? ""} className="mt-2 w-full rounded-xl border border-[#D8D5D6] px-4 py-3 font-normal" /></label><label className="text-sm font-semibold text-[#183A5A]">Returning consultation Cal.com URL<input name="returningBookingUrl" required type="url" placeholder="https://cal.com/..." defaultValue={settings?.returning_booking_url ?? ""} className="mt-2 w-full rounded-xl border border-[#D8D5D6] px-4 py-3 font-normal" /></label><label className="text-sm font-semibold text-[#183A5A] md:col-span-2">15-minute paying-client Cal.com URL<input name="clientSessionBookingUrl" type="url" placeholder="https://cal.com/..." defaultValue={settings?.client_session_booking_url ?? ""} className="mt-2 w-full rounded-xl border border-[#D8D5D6] px-4 py-3 font-normal" /></label><label className="flex items-center gap-3 text-sm font-semibold text-[#183A5A] md:col-span-2"><input name="clientSessionsActive" type="checkbox" defaultChecked={settings?.client_sessions_active ?? false} className="size-4 accent-[#D95B72]" /> Allow registered paying clients to book 15-minute sessions</label><div className="md:col-span-2 md:text-right"><button disabled={busy === "settings"} className="inline-flex items-center gap-2 rounded-full bg-[#183A5A] px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{busy === "settings" ? <LoaderCircle className="size-4 animate-spin" /> : <Save className="size-4" />} Save settings</button></div></form>
    </section>
  </div>;
}
