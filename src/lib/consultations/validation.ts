...
export const consultationSubmissionSchema = z.object({
  ...
  consultationType: z.string().trim().min(3).max(100),
  note: z.string().trim().max(1000).optional().or(z.literal("")),
  website: z.string().max(0).optional().or(z.literal(""))
});

export const consultationSettingsSchema = z.object({
  consultationName: z.string().trim().min(3).max(100),
  durationMinutes: z.coerce.number().int().min(10).max(480),
  priceDollars: z.coerce.number().min(1).max(100000),
  ...
  paidBookingUrl: calUrl,
  freeBookingUrl: calUrl,
  isActive: z.boolean()
});
