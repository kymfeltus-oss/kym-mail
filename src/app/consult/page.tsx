...
export default async function ConsultPage({ searchParams }: { searchParams: Promise<{ booking?: string }> }) {
  ...
  const [{ data: settings }, query] = await Promise.all([
    database.from("consultation_settings").select("cash_app_handle, payment_instructions, reference_instructions").eq("is_active", true).limit(1).maybeSingle(),
    searchParams
  ]);
  ...
  return <main className="min-h-screen bg-[#F7F3F2] px-4 py-6 sm:px-8 lg:py-10">
    <div className="mx-auto max-w-6xl">
      ...
      <section className="grid min-w-0 gap-10 py-12 lg:grid-cols-[.88fr_1.12fr] lg:items-start lg:py-20">
        ...
        {settings ? <div className="min-w-0"><div className="mb-5 min-w-0 rounded-3xl border border-[#E8E2E3] bg-[#FFFCFB] p-6 sm:p-8"><p className="text-xs font-semibold uppercase tracking-[.18em] text-[#D95B72]">Cash App payment</p><div className="mt-4 rounded-2xl bg-[#FFF3F4] p-5"><p className="break-words text-sm font-semibold text-[#A73D52]">Pay with Cash App: {settings.cash_app_handle}</p><p className="mt-2 whitespace-pre-line break-words text-sm leading-6 text-[#526173]">{settings.payment_instructions}</p>{settings.reference_instructions && <p className="mt-2 break-words text-sm leading-6 text-[#526173]">Reference: {settings.reference_instructions}</p>}</div><p className="mt-4 text-xs leading-5 text-[#7A8795]">KYM Mail does not connect to Cash App. Payment proof is reviewed manually by the owner.</p></div><ConsultationForm /></div> : <div className="min-w-0 rounded-3xl border border-[#E8E2E3] bg-white p-8"><h2 className="text-xl font-semibold text-[#183A5A]">Paid consultation intake is not open yet.</h2><p className="mt-3 text-sm leading-6 text-[#64748B]">The owner is finishing the payment instructions. Please check back soon.</p></div>}
      </section>
    </div>
  </main>;
}
