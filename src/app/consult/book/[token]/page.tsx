...
export default async function ConsultationBookingPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: request } = await database.from("consultation_requests").select("owner_id, client_name, consultation_type, payment_status, booking_token_expires_at, provider_booking_id").eq("booking_token_hash", hashConsultationToken(token)).maybeSingle();
