...
export async function GET(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: consultation } = await database.from("consultation_requests").select("id, owner_id, client_name, client_email, consultation_kind, payment_status, booking_token_expires_at, provider_booking_id").eq("status_token_hash", hashConsultationToken(token)).maybeSingle();
