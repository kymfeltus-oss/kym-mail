  ...
  return <AppShell email={owner.user.email} canSignOut={owner.mode === "authenticated"} active="projects">
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mt-8 grid min-w-0 gap-8 xl:grid-cols-[1.2fr_.8fr]">
        ...
        <aside className="min-w-0 space-y-6">{type === "JOB_SEARCH" && <section className="rounded-3xl border border-[#E8E2E3] bg-[#FFFCFB] p-5 shadow-[0_14px_42px_rgba(24,58,90,.06)] sm:p-6"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-semibold text-[#183A5A]">Jobs</h2>{status === "ACTIVE" && <Link href={`/app/jobs?project=${project.id}`} className="text-xs font-semibold text-[#A73D52]">Search Jobs</Link>}</div>{orderedProjectJobs.length ? <div className="mt-4 space-y-3">{orderedProjectJobs.map((job) => <Link key={job.id} href={`/app/jobs/saved/${job.id}`} className="block rounded-2xl border border-[#E8E2E3] p-4 transition hover:border-[#E7B8C1] hover:bg-[#FFF3F4]"><div className="flex items-start gap-3"><BriefcaseBusiness className="mt-0.5 size-4 shrink-0 text-[#D95B72]" /><div className="min-w-0"><p className="break-words text-sm font-semibold text-[#183A5A]">{job.title}</p><p className="mt-1 truncate text-xs text-[#64748B]">{job.company_name}{job.location_text ? ` · ${job.location_text}` : ""}</p></div></div></Link>)}</div> : <div className="mt-4 rounded-2xl bg-[#FFF3F4] p-4"><p className="text-sm font-semibold text-[#183A5A]">No saved jobs in this Project</p><p className="mt-1 text-xs leading-5 text-[#64748B]">Search real opportunities and save the right ones into this Project.</p></div>}</section>}<section className="rounded-3xl border border-[#E8E2E3] bg-[#FFFCFB] p-5 shadow-[0_14px_42px_rgba(24,58,90,.06)] sm:p-6"><h2 className="text-lg font-semibold text-[#183A5A]">Project activity</h2>{combinedActivity.length ? <ol className="mt-4 space-y-4">{combinedActivity.map((item) => <li key={item.id} className="border-l-2 border-[#F7DDE1] pl-4"><p className="text-sm font-semibold text-[#183A5A]">{item.label}</p>{item.activityType === "STATUS_CHANGED" && <p className="mt-1 text-xs text-[#64748B]">{String(item.details.from ?? "")} → {String(item.details.to ?? "")}</p>}<time className="mt-1 block text-[11px] text-[#94A3B8]">{formatMailTimestamp(item.occurredAt)}</time></li>)}</ol> : <p className="mt-3 text-sm leading-6 text-[#64748B]">No Project activity has been recorded.</p>}</section>
