...
export default async function ResumePreviewPage({ params }: { params: Promise<{ resumeId: string; versionId: string }> }) {
  ...
  const { data: version, error } = await owner.database.from("tailored_resume_versions").select("status, approved_at, content, tailored_resumes(job_opportunity_id)").eq("id", versionId).eq("resume_id", resumeId).eq("owner_id", owner.user.id).maybeSingle();
